...
export async function GET() {
  ...
  const res = await fetch(`${api}/orgs/${orgId}/integrations/creds`, { headers: { Authorization: bearer } });
...
export async function PATCH(req: NextRequest) {
  ...
  const orgId = meData?.orgs?.[0]?.orgId;
  if (!orgId) return NextResponse.json({ error: 'No organization found for user' }, { status: 400 });
  const res = await fetch(`${api}/orgs/${orgId}/integrations/creds`, { method: 'PATCH', headers: { Authorization: bearer, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
