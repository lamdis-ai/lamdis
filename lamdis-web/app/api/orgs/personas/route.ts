...
export async function GET(_req: NextRequest) {
  ...
  try {
    ...
    const resp = await fetch(`${api}/orgs/${orgId}/personas`, { headers: { Authorization: token }, cache: 'no-store' });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const resp = await fetch(`${api}/orgs/${orgId}/personas`, { method: 'POST', headers: { Authorization: token, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    const data = await resp.json().catch(()=>({}));
    return NextResponse.json(data, { status: resp.status });
  } catch (e:any) {
    return NextResponse.json({ error: e?.message || 'failed' }, { status: 500 });
