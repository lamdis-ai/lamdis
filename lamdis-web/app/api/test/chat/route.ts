...
export async function POST(req: NextRequest) {
  ...
  // Delegate to API which decrypts org OpenAI key
    const res = await fetch(`${api}/orgs/${orgId}/mcp/chat`, { method: 'POST', headers: { Authorization: bearer, 'Content-Type': 'application/json' }, body: JSON.stringify({ message, history: body.history || [] , version, debug }) });
