...
export default function EditPolicyPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [policyRes, catRes] = await Promise.all([
          authFetch(`/api/orgs/policies/${id}`),
        ...
        if (policyRes.ok) {
          ...
          setTags(data.tags || []);
          setPolicyTitle(data.title || "Policy");
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/library/policies/${id}`);
    } catch (err: any) {
      if (isAuthError(err)) {
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <Breadcrumbs
        items={[
          ...
          { label: policyTitle, href: `/dashboard/library/policies/${id}` },
          { label: "Edit" },
        ]}
      />
      ...
      <AuthErrorModal
        ...
        returnTo={`/dashboard/library/policies/${id}/edit`}
      />

      <h1 className="text-2xl font-semibold text-slate-100 mb-6">Edit Policy</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Updated compliance section for Q1 2026 regulations"
            className={inputCls}
          />
          <p className="text-xs text-slate-500 mt-1">Describe what changed for version history.</p>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => router.push(`/dashboard/library/policies/${id}`)}
