...
export function sanitizeInitialUserMessage(objective: string, proposed?: string): string | undefined {
  ...
  const banned = [
    ...
    /\bas a customer\b/i,
    /\bas a user\b/i,
    /\bseem (irritated|frustrated|angry|upset|annoyed)\b/i,
    /\bpersona\b/i,
    /\bmention that\b/i,
  ];
  ...
  let s = obj
    .replace(/\bas a (customer|user|caller|client|person|member|subscriber|account holder)\b[^.,;:)]*/gi, '')
    .replace(/\b(seem|act|be|sound|appear)\s+(irritated|frustrated|angry|upset|annoyed|confused|polite|calm|rude|impatient|demanding|aggressive|passive)[^.,;:)]*/gi, '')
    .replace(/\b(mention that|and mention|also mention)\b[^.,;:)]*/gi, '')
...
export async function synthesizeInitialUserMessage(opts: {
  ...
  - the "persona" field describes the TONE/STYLE to adopt (e.g., frustrated, polite). Apply it naturally — do NOT quote or repeat persona instructions in the message. The message must read like a real customer, not like an instruction.
