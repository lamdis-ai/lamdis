...
export async function runTestFile(body: z.infer<typeof runFileBodySchema>) {
  ...
  const schema = z.object({
    ...
    env: z.object({
      channel: z.enum(['http_chat', 'bedrock_chat']).default('http_chat'),
      ...
      if (!wfUrl) {
        ...
        if (chan === 'http_chat' && base) {
          ...
          const sendUser = async (userMsg: string, assistantInputMappings?: Record<string, any>, responseFieldPath?: string) => {
            ...
            // Extract reply using the configured responseFieldPath (supports nested paths)
            const fieldPath = responseFieldPath || 'reply';
            let rawReply: any = jr;
            ...
            logs.push({ t: now(), type: 'assistant_reply', content: String(replyTxt), latencyMs: dt, fullResponse: jr });
