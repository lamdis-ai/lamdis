...
export interface Org {
  ...
  org: {
    ...
    embodiedPlan?: string | null;
  };
}

/** Check whether the org has a given product active (non-null plan). */
export function orgHasProduct(org: Org | null, product: 'runs' | 'embodied'): boolean {
  if (!org?.org) return false;
  switch (product) {
    case 'runs': return !!org.org.currentPlan;
    case 'embodied': return !!org.org.embodiedPlan;
  }
}
...
export function OrgProvider({ children }: { children: ReactNode }) {
  ...
  const fetchOrgs = async () => {
    try {
      ...
      const validOrgs = (data.orgs || []).filter((o: Org) => o.orgId && o.org?.auth0OrgId);
      setOrgs(validOrgs);
